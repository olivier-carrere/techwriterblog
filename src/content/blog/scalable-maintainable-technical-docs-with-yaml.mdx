...
It’s true that tools like GitHub Desktop can help mitigate the issue and make the diff more human-readable. But under the hood, Git still considers that whole lines have changed, which makes it more difficult to handle the commit as a set of limited, self-contained changes.
