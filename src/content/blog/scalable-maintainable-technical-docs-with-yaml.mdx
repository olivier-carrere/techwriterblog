...
If you need a richer display, you can even switch to a **five-column table**—all without editing the source YAML.
...
flowchart TD
    ...
    %% Main steps
    B --> F1[Insert title]
    B --> F2[Insert short description]
    B --> F3[Create table with headers: Type, Brand, Cylinders, Viscosity grade, Price]
	B d1@--> H

    %% Row values (documents) in one subgraph
    subgraph RowValues["Table rows"]
      direction LR
      J@{ shape: docs, label: "Primary<br/>oil"} 
      K@{ shape: docs, label: "A1X"}
      L@{ shape: docs, label: "One-cylinder<br/>engines"}
      M@{ shape: docs, label: "0W-20"}
      N@{ shape: docs, label: "$15.00"}
    end

    %% Loop box placed at very bottom
    H[For each YAML entry, create a table row]
    H h1@-.-> J
    H h2@-.-> K
    H h3@-.-> L
    H h4@-.-> M
    ...
    subgraph RowHeaders["Table header"]
      direction LR
	  H1[Type]
	  H2[Brand]
	  H3[Cylinders]
	  H4[Viscosity<br/>grade]
	  H5[Price]
    end
    ...
    class B topic;
    class F1,F2,F3, step;
    class H1,H2,H3,H4,H5 substep;
    class J,K,L,M,N documents;
