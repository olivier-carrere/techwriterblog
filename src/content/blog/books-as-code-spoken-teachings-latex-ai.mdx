...
2. **Verify what AI proposes against your own technical knowledge.** The model will generate plausible-looking LaTeX; whether it's idiomatic is a different question. Having [built a LaTeX document from scratch](https://www.overleaf.com/latex/templates/leaflet/ysdkbbhctfpc) a few years earlier made it possible to spot when a generated solution was technically correct but roundabout, or when a package choice would cause conflicts downstream. Prior hands-on experience isn't a prerequisite — but without it, you're accepting the output on trust rather than judgment.
