...
snippet: "How a fully AI-assisted, open-source LaTeX pipeline turned twenty years of a teacher's recorded talks into camera-ready, multi-format, multi-language books — one source, many outputs."
image: {
    src: "/images/blog/shinjinmei-docs-as-code.webp",
    alt: "A single plain-text source branching into print, screen, and audio outputs — the Shinjinmei docs-as-code project"
}
...
![A single plain-text source branching into print, screen, and audio outputs — the Shinjinmei docs-as-code project](/images/blog/shinjinmei-docs-as-code.webp)
...
> **Note:** the A4 two-column edition costs nothing extra to produce — it's just another target — yet it suits institutional use well: libraries, reading groups, and anyone printing on office hardware.
...
What makes it a *loop* rather than a one-shot is direction: AI output feeds back into the LaTeX sources, which are re-checked on the next pass. The machine drafts and flags; the human decides and commits. It's the same split I leaned on in an [AI-assisted workflow for translating legacy content](https://redaction-technique.org/blog/ai-translation-legacy-technical-docs).
...
That is the whole project in one gesture: the book is not a replacement for the talks, it's **an index into them**. Print and digital stop competing and start reinforcing each other — text you can study, a voice you can return to. The same bridge between archive and living attention runs through [turning a corpus into living knowledge](https://redaction-technique.org/blog/transforming-corpus-ai-living-knowledge); here it's just a square of black-and-white ink, scanned off the page.
...
Open sources reinforce it. Anyone can rebuild the book from the repository, and the commit hash in each copy ties the artifact back to the exact text that produced it. Inspect the source, check the hash, print your own — the verification path is open from end to end.
